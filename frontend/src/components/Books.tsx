...
export default function Books() {
  ...
  //     NProgress.start();
  //     router.push(`/reader/${slugify(title ?? "")}/${id ?? ""}`);
  //   };
  ...
  const deleteInIndexedDB = async (id: number, stateId: string) => {
    console.log("id: ", id);
    console.log("Deleteid: ", id);
    deleteBookFromIndexedDB(id);
    const updatedBooksEffect = booksEffect?.filter((book) => book.id !== stateId);
    setBooksEffect(updatedBooksEffect ?? null);
  };
  ...
  return (
    <div className="border-t border-black/[0.18] mx-auto w-full">
      {booksEffect?.length === 0 ? (
        <p className="mt-10">No books available.</p>
      ) : (
                        ...
                        <div className="modal-actions shadow bg-white absolute bottom-6 rounded z-50 -right-33 py-2 w-[170px] flex flex-col gap-1">
                          ...
                          <span
                            ...
                            onClick={() =>
                              index > 1
                                ? deleteInIndexedDB(
                                    Number(book.bookIndexedDBId),
                                    book.id ?? ""
                                  )
                                : deleteInFileSystem(book.filename ?? "", book.id ?? "")
                            }
                          >
                            Delete this book
                          </span>
